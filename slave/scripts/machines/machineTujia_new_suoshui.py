        ...
        try:
            ...
            for _ in range(looknum):
                ...
                if not random.randint(0, 19):
                    try:
                        content = WebDriverWait(dr, 5).until(lambda d: d.find_element_by_id("com.tujiaapp.tujia:id/tv_comment_count"))
                        ...
                        if int(contentnum) > 5:
                            ...
                            time.sleep(2)
                            contenttext = WebDriverWait(dr, 30).until(lambda d: d.find_elements_by_id("com.tujiaapp.tujia:id/tv_content"))
                            comtext = contenttext[random.randint(0, contenttext.__len__()-1)].text
                            WebDriverWait(dr, 30).until(lambda d: d.find_element_by_id("com.tujiaapp.tujia:id/tv_to_comment")).click()
                            time.sleep(1)
                            edit = WebDriverWait(dr, 30).until(lambda d: d.find_element_by_class_name("android.widget.EditText"))
                            edit.send_keys(comtext)
                            time.sleep(1)
                            WebDriverWait(dr, 30).until(lambda d: d.find_element_by_id("com.tujiaapp.tujia:id/view_comment_sure")).click()
                            time.sleep(1)
                            WebDriverWait(dr, 30).until(lambda d: d.find_element_by_id("com.tujiaapp.tujia:id/iv_close_fragment")).click()
                            time.sleep(1)
                    except TimeoutException:
                        pass
                dr.press_keycode(4)
