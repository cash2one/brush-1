...
class Machinex(Machine):
    ...
    def signup(self):
        dr = self.driver
        WebDriverWait(dr, 15).until(lambda d: d.find_element_by_id("com.yixia.videoeditor:id/login_weibo_button")).click()
        time.sleep(1)
        #进入微博登录界面
        ...
        try:
            for _ in range(5):
                ...
                try:
                    ...
                    break
            #注册成功页面检测
            WebDriverWait(dr, 60).until(lambda d: d.find_element_by_id("com.sina.weibo:id/bnLogin")).click()
            time.sleep(1)
