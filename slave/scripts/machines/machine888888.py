...
class Machinex(Machine):
    def __init__(self, driver, code_platform, fm_uname, fm_pwd):
        ...
        self.appname = "半岛"       #app名字
        self.appname_en = "bandao"     #记录文件用缩写英文名
        self.imei = None
    ...
    def initdata(self):
        ...
        if self.code_platform == "feima":
            ...
            self.code = Ailezan(self.code_user, self.code_pwd, 17470)
    ...
    def begin(self):
        ...
        time.sleep(10)
        #检测已进入app
        WebDriverWait(dr, 60).until(lambda d: d.find_element_by_id("com.bandaorongmeiti.news:id/main_container"))
        ...
        WebDriverWait(dr, 30).until(lambda d: d.find_element_by_id("com.bandaorongmeiti.news:id/splashguide_start")).click()
        time.sleep(5)
        WebDriverWait(dr, 30).until(lambda d: d.find_element_by_id("com.bandaorongmeiti.news:id/tabs_guide3")).click()
        time.sleep(1)
        #注册率
        # sign_rate = random.randint(1, 10000)
        # if sign_rate <= 10000:
        WebDriverWait(dr, 30).until(lambda d: d.find_element_by_id("com.bandaorongmeiti.news:id/tabs_guide5")).click()
        time.sleep(1)
        WebDriverWait(dr, 30).until(lambda d: d.find_element_by_id("com.bandaorongmeiti.news:id/my_face")).click()
        time.sleep(1)
    ...
    def signup(self):
        ...
        try:
            #进入注册页面
            WebDriverWait(dr, 10).until(lambda d: d.find_element_by_id("com.bandaorongmeiti.news:id/usrlogin_register")).click()
            time.sleep(1)
            #选择接码平台获取手机号码
            ...
            time.sleep(1)
            #点击获取验证码按钮
            WebDriverWait(dr, 10).until(lambda d: d.find_element_by_id("com.bandaorongmeiti.news:id/usrreg_sendcode")).click()
            time.sleep(5)
            #输入密码
            ...
            time.sleep(1)
            edts[4].send_keys(888888)
            time.sleep(1)
            #选择接码平台获取验证码
            #【半岛都市报】您正在申请注册半岛,验证码:7133（10分钟内有效）。半岛是由《半岛都市报》出品的唯一官方客户端，为了保护您的账号安全，验证短信请勿转发给其他人。(来自106906857000312)
            regrex = r'验证码:(\d+)'
            ...
            time.sleep(1)
            #点击完成按钮按钮
            WebDriverWait(dr, 10).until(lambda d: d.find_element_by_id("com.bandaorongmeiti.news:id/usrreg_btn")).click()
            time.sleep(1)
            #检测注册成功进入下一步
...
class Machinex2(Machine):
    ...
    def login(self):
        ...
        if user and pwd:
            ...
            time.sleep(1)
            WebDriverWait(dr, 15).until(lambda d: d.find_element_by_id("com.bandaorongmeiti.news:id/usrlogin_login")).click()
            time.sleep(5)
            WebDriverWait(dr, 60).until(lambda d: d.find_element_by_id("android:id/button1")).click()
        time.sleep(random.randint(15, 30))
        return self.ends
