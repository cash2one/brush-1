...
class Machinex(Machine):
    def __init__(self, driver, code_platform, fm_uname, fm_pwd):
        ...
        self.appname = "吉银财富"       #app名字
        self.appname_en = "jiyin"     #记录文件用缩写英文名
        self.imei = None
    ...
    def initdata(self):
        ...
        if self.code_platform == "feima":
            ...
            self.code = Shenhua(self.code_user, self.code_pwd, 97851)
    ...
    def begin(self):
        ...
        if sign_rate <= 10000:
            WebDriverWait(dr, 30).until(lambda d: d.find_element_by_id("com.pafinancialtech.jilinbank:id/head_image")).click()
            time.sleep(1)
    ...
    def after_signup(self):
        ...
        if not random.randint(0, 9):
            try:
                WebDriverWait(dr, 15).until(lambda d: d.find_elements_by_class_name("android.widget.ImageButton"))[4].click()
                time.sleep(1)
                WebDriverWait(dr, 10).until(lambda d: d.find_element_by_id("com.pafinancialtech.jilinbank:id/userThumbnail")).click()
                time.sleep(1)
                WebDriverWait(dr, 10).until(lambda d: d.find_element_by_id("com.pafinancialtech.jilinbank:id/rl_personalinformation_icon")).click()
                time.sleep(1)
                ...
                time.sleep(1)
                WebDriverWait(dr, 15).until(lambda d: d.find_element_by_id("com.pafinancialtech.jilinbank:id/ok")).click()
                time.sleep(5)
                #保存信息按钮
                WebDriverWait(dr, 15).until(lambda d: d.find_element_by_id("com.pafinancialtech.jilinbank:id/iv_title_back_button")).click()
                time.sleep(10)
                #检测信息保存完毕跳转页面
                WebDriverWait(dr, 60).until(lambda d: d.find_element_by_id("com.pafinancialtech.jilinbank:id/iv_title_back_button")).click()
                time.sleep(1)
            except TimeoutException:
                pass
        #记录帐号密码
            ...
            while self.readnum:
                ...
                if random_read == 0:
                    if self.ismenu1:
                        print("goto menu1")
                        try:
                            WebDriverWait(dr, 15).until(lambda d: d.find_elements_by_class_name("android.widget.ImageButton"))[0].click()
                            time.sleep(5)
...
class Machinex2(Machine):
    ...
    def login(self):
        ...
        if user and pwd:
            WebDriverWait(dr, 15).until(lambda d: d.find_element_by_id("com.pafinancialtech.jilinbank:id/head_image")).click()
            time.sleep(1)
            ...
            time.sleep(1)
            WebDriverWait(dr, 15).until(lambda d: d.find_element_by_id("com.pafinancialtech.jilinbank:id/login_btn_login")).click()
            time.sleep(5)
            try:
                WebDriverWait(dr, 60).until(lambda d: d.find_element_by_name("要使用更便捷的登录方式吗？"))
                WebDriverWait(dr, 15).until(lambda d: d.find_elements_by_class_name("android.widget.TextView"))[2].click()
                time.sleep(5)
            except TimeoutException:
                pass
            dr.press_keycode(4)
