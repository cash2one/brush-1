...
class Machinex(Machine):
    ...
    def location(self):
        ...
        time.sleep(1)
        #模拟定位
        WebDriverWait(dr, 30).until(lambda d: d.find_element_by_name("模拟位置")).click()
        time.sleep(1)
        WebDriverWait(dr, 30).until(lambda d: d.find_element_by_name(self.appname)).click()
        time.sleep(1)
        # try:
        #     WebDriverWait(dr, 5).until(lambda d: d.find_element_by_xpath("//android.widget.Switch[@checked='false']")).click()
        #     time.sleep(1)
        ...
        for x in range(10):
            ...
            if edts[0].text == "":
                break
        edts[0].send_keys(random.randint(27, 38)+random.randint(1, 10)/10)
        time.sleep(1)
        #输入纬度
        for x in range(10):
            edts[1].click()
            ...
            if edts[1].text == "":
                break
        edts[1].send_keys(random.randint(107, 117)+random.randint(1, 10)/10)
        time.sleep(1)
        WebDriverWait(dr, 30).until(lambda d: d.find_element_by_id("com.rong.xposed.fakelocation:id/chk_instant_update")).click()
        time.sleep(1)
        dr.press_keycode(4)
        time.sleep(1)
        return self.begin
