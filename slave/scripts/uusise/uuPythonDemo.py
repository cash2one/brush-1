...
pic_file_path = os.path.join(os.path.dirname(__file__), 'test_pics', 'caca.png')   #测试图片文件
...
# s_id  = 0                               # 软件ID
# s_key = "2e73fd59aa1044d9af697ade5abf214b"  # 软件Key 获取方式：http://dll.uuwise.com/index.php?n=ApiDoc.GetSoftIDandKEY
...
#DLL 文件MD5值校验
#用处：近期有不法份子采用替换官方dll文件的方式，极大的破坏了开发者的利益
#用户使用替换过的DLL，导致开发者分成变成别人的，利益受损，
#所以建议所有开发者在软件里面增加校验官方MD5值的函数

dllMd5 = getFileMd5(UUDLL)    #api文件的MD5值
dllCRC32 = getFileCRC(UUDLL)  #API文件的CRC32值
randChar = hashlib.md5(random.choice('abcdefghijklmnopqrstuvwxyz!@#$%^&*()').encode()).hexdigest()     #随机字符串，用于让返回来的结果随机
softVerifyKey = "5E7EA632-2529-4B35-9DB8-BB633C0F9646"        #在开发者后台软件列表内获取，请勿泄露此KEY。图例：http://dll.uuwise.com/index.php?n=ApiDoc.GetSoftIDandKEY


# checkStatus = hashlib.md5(('%d%s%s%s%s' % (s_id, (softVerifyKey.upper()), (randChar.upper()), (dllMd5[1].upper()), (dllCRC32[1].upper()))).encode()).hexdigest()       #服务器返回来的值与此值对应一至则表示成功
...
user = c_wchar_p("xiaoxiaozhuan")  # 授权用户名
passwd = c_wchar_p("meiriq2014")  # 授权密码
...
code_id = recognizeByCodeTypeAndPath(c_wchar_p(pic_file_path), c_int(1005), result)       #图片类型联系QQ：800069852
if code_id <= 0:
